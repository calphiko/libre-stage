  ...
  <div class="card bg-surface-2 rounded-lg shadow-lg md:border p-2 md:p-5">
    <div class="flex flex-col gap-4">
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <ul class="list-disc list-inside space-y-1 text-sm">
                ...
                <li><strong>Probe oeffnen:</strong> Klicke auf eine Probe, um die Details im Modal zu sehen</li>
            ...
            <div>
              ...
              <ul class="list-disc list-inside space-y-1 text-sm">
                ...
                <li><strong>Suche (innen):</strong> Innerhalb des geoeffneten Probe-Details-Modals kannst du die Songs direkt durchsuchen</li>
              </ul>
            </div>
      ...
      {#if rehearsals.length > 0}
        <div class="flex border-b border-surface-300 dark:border-surface-600 mb-3 gap-1 text-sm">
          <button onclick={() => tabSet = 0} class="px-3 py-1.5 rounded-t-lg transition-colors {tabSet === 0 ? 'bg-surface-200 dark:bg-surface-700 font-bold border-b-2 border-primary-500' : 'hover:bg-surface-100 dark:hover:bg-surface-800'} {upcomingRehearsals.length > 0 ? 'font-bold' : ''}">
        ...
        <div class="mt-2">
          {#if tabSet === 0}
            {#if upcomingRehearsals.length === 0}
              <div class="rounded-xl bg-success-100 text-success-900 p-3 mt-4 shadow text-center text-sm">
            ...
            {:else}
              <div class="mt-2">
                {#each upcomingRehearsals as reh (reh.id)}
                  <RehearsalCard
                    {reh}
                    onopen={() => openRehearsalDetails(reh)}
                  />
                {/each}
              </div>
            {/if}
          {:else}
            ...
            {:else}
              <!-- Suchfeld -->
              <div class="mt-2 mb-2">
                <div class="flex items-center gap-1.5 border border-outline-variant rounded-lg px-2.5 py-1.5 bg-surface-1">
