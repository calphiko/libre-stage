...
export async function getUserTodos(token) {
    ...
    return {
        ...
        gigChecklistTodos: todos.gig_checklist_todos ?? [],
    };
}
...
export async function getPlaylistConfig() {
    const res = await fetchWithAuth(`${API_URL}/playlist/config`, { method: 'GET' });
    if (!res.ok) throw new Error('Playlist-Config konnte nicht geladen werden');
    return res.json();
}

export async function getPlaylistSongs() {
    const res = await fetchWithAuth(`${API_URL}/playlist/songs`, { method: 'GET' });
    if (!res.ok) throw new Error('Playlist konnte nicht geladen werden');
    return res.json();
}

export async function addPlaylistSong(data) {
    const res = await fetchWithAuth(`${API_URL}/playlist/songs`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
    });
    if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err.detail ?? 'Song konnte nicht hinzugefügt werden');
    }
    return res.json();
}

export async function deletePlaylistSong(songId) {
    const res = await fetchWithAuth(`${API_URL}/playlist/songs/${songId}`, { method: 'DELETE' });
    if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err.detail ?? 'Song konnte nicht entfernt werden');
    }
    return res.json();
}

export async function ratePlaylistSong(songId, rating, comment = null) {
    const res = await fetchWithAuth(`${API_URL}/playlist/songs/${songId}/rate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err.detail ?? 'Bewertung konnte nicht gespeichert werden');
    }
    return res.json();
}

export async function deletePlaylistRating(songId) {
    const res = await fetchWithAuth(`${API_URL}/playlist/songs/${songId}/rate`, { method: 'DELETE' });
    if (!res.ok) throw new Error('Bewertung konnte nicht entfernt werden');
    return res.json();
}

export async function searchStreamingTracks(query, platform = 'spotify') {
    const params = new URLSearchParams({ q: query, platform });
    const res = await fetchWithAuth(`${API_URL}/playlist/search?${params}`, { method: 'GET' });
    if (!res.ok) {
        const err = await res.json().catch(() => ({}));
...
export async function triggerPlaylistSync() {
    const res = await fetchWithAuth(`${API_URL}/playlist/sync`, { method: 'POST' });
    if (!res.ok) {
        const err = await res.json().catch(() => ({}));
...
export async function getPlaylistSyncLog() {
    const res = await fetchWithAuth(`${API_URL}/playlist/sync/log`, { method: 'GET' });
    if (!res.ok) throw new Error('Sync-Log konnte nicht geladen werden');
    return res.json();
}

export async function getStreamingOAuthStatus() {
    const res = await fetchWithAuth(`${API_URL}/playlist/oauth/status`, { method: 'GET' });
    if (!res.ok) throw new Error('OAuth-Status konnte nicht geladen werden');
    return res.json();
}

export async function disconnectStreamingPlatform(platform) {
    const res = await fetchWithAuth(`${API_URL}/playlist/oauth/${platform}/disconnect`, { method: 'DELETE' });
    if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err.detail ?? 'Trennen fehlgeschlagen');
    }
    return res.json();
}
