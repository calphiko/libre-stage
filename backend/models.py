...
class PlaylistSong(Base):
    """
    A song in the band's inspiration playlist.
    ...
    Attributes:
        ...
        tidal_track_id (str | None): Tidal track ID.
        cover_url (str | None): Album cover image URL.
        proposed_by (int): FK to :class:`User` who proposed the song.
        created_at (datetime): When the song was added.
        is_synced_spotify (bool): Whether the song is in the Spotify playlist.
        is_synced_tidal (bool): Whether the song is in the Tidal playlist.
    """
    ...
    spotify_track_id = Column(String(64), nullable=True)
    ...
    is_synced_spotify = Column(Boolean, default=False, nullable=False, server_default="0")
    is_synced_tidal = Column(Boolean, default=False, nullable=False, server_default="0")

    proposer = relationship('User', foreign_keys=[proposed_by])
    ratings = relationship('PlaylistRating', back_populates='song', cascade="all, delete-orphan")
    sync_logs = relationship('PlaylistSyncLog', back_populates='song', cascade="all, delete-orphan")


class PlaylistRating(Base):
    """
    A user's rating for a song in the inspiration playlist.

    Attributes:
        id (int): Primary key.
        song_id (int): FK to :class:`PlaylistSong`.
        user_id (int): FK to :class:`User`.
    ...
    """
    __tablename__ = "playlist_ratings"
    __table_args__ = (
        UniqueConstraint('song_id', 'user_id', name='_playlist_rating_uc'),
    )

    id = Column(Integer, primary_key=True, index=True)
    song_id = Column(Integer, ForeignKey('playlist_songs.id'), nullable=False)
    user_id = Column(Integer, ForeignKey('users.id'), nullable=False)
    ...
    song = relationship('PlaylistSong', back_populates='ratings')
    user = relationship('User')


class PlaylistSyncLog(Base):
    """
    Log entry for a playlist synchronization action.

    Attributes:
        id (int): Primary key.
        song_id (int | None): FK to :class:`PlaylistSong` (None for full-sync entries).
        platform (str): Platform name (``spotify`` or ``tidal``).
        action (str): Action performed (``added``, ``removed``, ``failed``, ``full_sync``).
        timestamp (datetime): When the action occurred.
        error_message (str | None): Error details if action failed.
    """
    __tablename__ = "playlist_sync_logs"

    id = Column(Integer, primary_key=True, index=True)
    song_id = Column(Integer, ForeignKey('playlist_songs.id'), nullable=True)
    platform = Column(String(32), nullable=False)
    action = Column(String(32), nullable=False)
    timestamp = Column(DateTime, default=lambda: datetime.now(timezone.utc))
    error_message = Column(Text, nullable=True)

    song = relationship('PlaylistSong', back_populates='sync_logs')


class StreamingToken(Base):
    """
    Per-platform streaming credentials and OAuth tokens.

    Credentials (client_id, client_secret, playlist_id) and OAuth tokens
    are all stored here so nothing sensitive lives in appConfig.json.

    Attributes:
        id (int): Primary key.
        platform (str): Platform name (``spotify`` or ``tidal``), unique.
        client_id (str | None): OAuth application client ID (plain text).
        client_secret_enc (str | None): Fernet-encrypted client secret.
        playlist_id (str | None): Target playlist ID / UUID on the platform.
        access_token_enc (str | None): Fernet-encrypted access token.
        refresh_token_enc (str | None): Fernet-encrypted refresh token.
        token_expires_at (datetime | None): Access token expiry (UTC).
        scope (str | None): Granted OAuth scopes.
        updated_at (datetime): Last update timestamp.
    """
    __tablename__ = "streaming_tokens"

    id = Column(Integer, primary_key=True, index=True)
    platform = Column(String(32), nullable=False, unique=True)
    # ── Credentials (stored in DB, not in appConfig.json) ──────────────────
    client_id = Column(String(256), nullable=True)
    client_secret_enc = Column(Text, nullable=True)   # Fernet-encrypted
    playlist_id = Column(String(256), nullable=True)
    # ── OAuth tokens ────────────────────────────────────────────────────────
